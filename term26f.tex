...
contract paradigm.
Finally, we show how to extend the equivalence results to a larger class of programs.

\III

\NI
KRA (1-03-26): THE LAST SENTENCE ABOVE WAS ADDED
...
KRA (1-03-26):BEGIN REVISED (and moved here)
...
$p \ra t \geq 0$.  We call the corresponding rules the LOOP $B$ rules.
...
ERO (27 Feb 2026): NEW DISCUSSION -- WHERE TO PLACE IT ?
...
be derived from the LOOP~II rule, we need the auxiliary rule
...
This proof is by induction on the structure of while programs.
The crucial case is that of a while loop. By induction hypothesis,
we can deduce from 
\[
...
However, the  $var(t) \subseteq var(B) \cup var(S_0)$ is too restrictive 
for some applications. In~\cite{ABO09}, we treat in Section 3.9 the case study
``Partitioning an Array''. The partitioning program {\it PART} has nested
while loops. When proving the termination of the inner loops we needed
bound functions that refer to a variable that occur outside of these loops
...
Equivalence Theorem \ref{thm:abo} comprises six equivalences. However,
thanks to the transitivity of the equivalence relation, it suffices to
prove three of them. We found it most convenient to deal with the
equivalence of the following two pairs of proof systems: (II, III) and
(III, IV). To this end we, invoke four times
Lemma~\ref{lem:equ}, each time establishing its assumptions for an
appropriate pair of LOOP rules $R_1$ and $R_2$ and a specific set of
rules $\AUX$ drawn from the set of auxiliary rules listed in the
previous section, augmented with the CONSEQUENCE rule (that also is an
adaptation rule).  The details are given in Appendix~\ref{sec:proof}.
...
result, since it concerns on the class of \textbf{while}
programs. Here we consider how to extend the Equivalence
...
simplicity we assume that procedures do not have global veriables and
...
\HT{x=x_0 \wedge y=y_0}{swap(x,y)}{y=x_0 \wedge x=y_0} \vdash  \\
 ...
 \label{equ:gcd}
\end{equation}
That is, we want to establish total correctness of the $\GCD$ program
under the stated assumption about the procedure call \emph{swap(x,y)}.
It is clear how to prove the above claim in any of the proof systems
II-V: in the proof we can simply use the assumed property of
\emph{swap(x,y)} as an additional axiom. However, care has to be
exercised because some auxiliary rules are needed.
...
$p$.  To establish it we need another auxiliary rule (see \cite[page
468]{ABO09}):

\III

\NI
RULE \label{rul:inv} INVARIANCE
\[ 
 \frac{ \HT{p}{S}{q} }
      { \HT{p \A r}{S}{q \A r}         } 
\]
where no variable in $\mathit{free}(r)$ can change in $S$.
\III

Applying it to the assumption
\[
  \HT{x=x_0 \wedge y=y_0}{swap(x,y)}{y=x_0 \wedge x=y_0}
\]
we obtain first
\[
  \HT{x=x_0 \wedge y=y_0 \wedge p(x_0,y_0)}{swap(x,y)}{y=x_0 \wedge x=y_0 \wedge p(x_0,y_0)},
\]
...
we get
\[
  \HT{\te x_0, y_0: (x=x_0 \wedge y=y_0 \wedge p(x_0,y_0))}{swap(x,y)}{p(y,x)},
\]
...
% \qquad\qquad                                       
% \= \C{p(x,y)} \\
% \> \C{\te x_0, y_0: x=x_0 \wedge y=y_0 \wedge p(x_0,y_0)} \\
% \> \C{x=x_0 \wedge y=y_0 \wedge p(x_0,y_0)} \\
% \>  $swap(x,y) $                         \\
% \> \C{y=x_0 \wedge x=y_0 \wedge p(x_0,y_0)} \\
% \> \C{p(y,x)}
% \end{tabbing}
...
\]
 ...
 \begin{tabbing}
 \qquad\qquad                                       
\= \C{x=x_0 \wedge y=y_0 \wedge x>0 \wedge y>0} \\
\> \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0} \\
\> \C{\textbf{inv} : gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0} \C{\textbf{bd}: x+y}    \\
\> $\WD{x \neq y}$                          \\
\>  \qquad \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0 \wedge z = x+y \wedge x \neq y} \\
\>  \qquad $\IF x < y \THEN$                         \\
\> \qquad \qquad \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0 \wedge z = x+y \wedge x < y} \\
\>  \qquad \qquad  $swap(x,y) $                         \\
\> \qquad \qquad \C{gcd(y,x)=gcd(x_0, y_0)  \wedge y>0 \wedge x>0 \wedge z = y+x \wedge y < x} \\
\> \qquad \qquad \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0 \wedge z = x+y \wedge x > y} \\
\>  \qquad \FI; \\
\>  \qquad \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0 \wedge z = x+y \wedge x > y} \\
\>  \qquad $x:= x-y$                         \\
\>  \qquad \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0 \wedge z < x+y} \\
\>  \OD                                      \\
\> \C{gcd(x,y)=gcd(x_0, y_0)  \wedge x>0 \wedge y>0 \wedge x=y} \\
\> \C{x=y = gcd(x,y) \wedge gcd(x,y)=gcd(x_0, y_0)} \\
\> \C{x=y=gcd(x_0, y_0)} 
\end{tabbing}
...
\]
where each $\phi_i$ is an assertion or a correctness formula referring
to a procedure call and $S$ is a program that may contain any of the
procedure calls appearing in some $\phi_i$.

It is clear how to modify all considered proof rules to deal with the
conditional correctness formulas: it suffices to append each
correctness formula by `$\phi_1, \dots, \phi_n \vdash$'.  This yields
the proof systems CI-CIV.
...
Additionally, we could add an appropriate rule that allows us to
...
for the simple reason that in this proof system there is no way to
prove the premise
\[
  \begin{array}{l}  
    \HT{p \A B}{S}{p}, \HT{p \A B \A t=z}{S}{t<z}, p \ra t \geq 0 \vdash  \\
  ...
  \end{array}    
\]
of the LOOP II rule. (Such a proof would in fact show that in the
proof system I the LOOP II rule is derivable from the LOOP III rule,
which is not the case.)  Note, however, that this premise directly
follows proved using the CONJUNCTION rule.
In fact, the following counterpart of the Equivalence Theorem~\ref{thm:abo}
holds.

KRA: CLARIFY THE SET $\AUX$
...
KRA:  TO EXPAND
  ...
  It is easy to see check that the proof of Theorem~\ref{thm:abo} remains valid after replacing each
  correctness formula by a conditional one.
\HB
\end{proof}
...
We first deal with the derivability.
% instantiating part (i) of Lemma~\ref{lem:equ}.
...
\NI
$(i)$ It suffices to show how to derive from the premise
\[
  \HT{p \A B \A t=z}{S}{p \A t<z}
\]
...
\NI
$(ii)$ It suffices to note that from the first two premises 
\[
...
We now turn to the proof of Lemma~\ref{lem:admissibility}, 
...
using Lemma~\ref{lem:derivability} and Lemma~\ref{lem:admissibility}.
...
A straightforward proof by induction on the structure of $S$
shows that
\[
