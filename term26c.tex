...
Theorem \ref{thm:abo} comprises six equivalences. However, thanks to the
...
The details are given in Appendix~\ref{sec:proof}.

To close this discussion note the following consequence of Theorem~\ref{thm:abo}
...
\NI
KRA: IF WE KEEP IT, SHOULD WE PROVE IT?  \\
*** ERO: PROOF ADDED.


\begin{proof}
We show this for the case $\alpha$ = II and $\beta$ = III.
By Theorem~\ref{thm:abo}, proof system~II is equivalent to proof system~III,
...
the sense of Theorem \ref{thm:abo}, they lead to different proofs of
...
As explained in Subsection \ref{subsec:structure}, to prove 
Theorem~\ref{thm:abo} (The proof systems II--V are equivalent.),
...
in $p$ or $t$ (note that by assumption $\mbox{\it var}(t)\subseteq \mbox{\it pvar}$), we have
\[
  p \A B \to (p \A B \A \te z: t=z) \to  \te z: (p \A B \A t=z),
\]
(note that by definition also $\mbox{\it var}(B)\subseteq \mbox{\it pvar}$).
By the CONSEQUENCE rule, we derive also the first premise
...
\[
 \HT{p'(x)\wedge B}{S}{\exists y: p'(y)\wedge y< x}
\]
where $p'(x)$ results from replacing every logical variable
...
\]
Since  $(\exists z:p'(x))\leftrightarrow p$, 
a trivial  application of the CONSEQUENCE rule
...
we have $x \not \equiv u$ and $x \not\in var(t)$ and $u \not\in var(t_0)$.
So the substitutions commute: $q_1[x:=t_0][u:=t] \equiv q_1[u:=t][x:=t_0]$.
Thus we have
\[
 \HT{q_1[u:=t][x:=t_0]}{S}{q_1[x:=t_0]}.
\]
The implications 
$p[x:=t_0] \ra (q_1[u:=t][x:=t_0])$ and $q_1[x:=t_0] \ra q[x:=t_0]$ hold.
So the CONSEQUENCE rule yields
...
We show how to obtain Theorem~\ref{thm:abo} by instantiating Lemma~\ref{lem:equ}
using Lemma~\ref{lem:derivability} and Lemma~\ref{lem:admissibility}.
We treat the following case: proof system II $\approx$ proof system III.

Using Lemma~\ref{lem:derivability}(i) and Lemma~\ref{lem:admissibility}(ii), 
we instantiate Lemma~\ref{lem:equ} with $T$ being the axiom and the proof rules of 
