...
where we prove that they are all inessential.  \texttt{*** ToDo ***}
...
*** ERO renamed $z$ into $z_0$ to avoid clashes with other $z$ in proofs
\[ 
 \frac{ \HT{p}{S}{q}          }
      { \HT{\te z_0:p}{S}{q}    }
\]
...
equivalence of the following three pairs of proof systems: (II, III),
(III, IV), and (II, V).  To this end we, invoke six times
...
\NI
*** ERO: PROOF ADDED. \\
KRA: (13-02) I REPLACED IT BY A GENERAL PROOF. 
...
\begin{proof}
  ...
  $\alpha, \beta \in \{\mathrm{II, III, IV, V}\}$ we have
  $T \cup \alpha \approx T \cup \beta$ and hence
  $T \cup \beta \approx T \cup \{\alpha, \beta\}$.  So by
  Lemma~\ref{lem:AUX}(i) the LOOP $\alpha$ rule is admissible in proof
  system $T \cup \beta$.
...
of a bound.  But also the other rules lead to different proofs.  The
...
for each of the six pair of rules
\[
...
\begin{proof}
  KRA: (13-02-26) THE CASES (i)-(iv) SHORTENED
...
\NI
KRA: (13-02-26) SUBSTANTIALLY REWRITTEN
...
In all cases we assume that $z_0 \not\in var(S) \cup \mathit{free}(q)$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\NI
$(vi)$ The {\rm SUBSTITUTION} rule is admissible in the proof system {\rm II}.
...
followed by one final application of the CONSEQUENCE rule
according to Lemma~\ref{lemma:1}.
In all cases we assume that $x$ is a logical variable with $x \not\in var(S)$
and $var(t_0) \cap var(S) = \emptyset$.
\III
...
By the ASSIGNMENT axiom, also
\[
 \HT{q_1[x:=t_0][u:=t]}{S}{q_1[x:=t_0]}.
\]
By the assumptions on $x$ and $t_0$, 
we have $x \not \equiv u$ and $x \not\in var(t)$ and $u \not\in var(t_0)$.
So the substitutions commute, that is, $q_1[x:=t_0][u:=t] \equiv q_1[u:=t][x:=t_0]$.
Thus we have
\[
 \HT{q_1[u:=t][x:=t_0]}{S}{q_1[x:=t_0]}.
\]
The implications
\[
  \mbox{$p[x:=t_0] \ra (q_1[u:=t][x:=t_0])$ and $q_1[x:=t_0] \ra q[x:=t_0]$}
\]
hold.
So the CONSEQUENCE rule yields
$\vdash_{\rm{II}}  \HT{p[x:=t_0]}{S}{q[x:=t_0]}$,
as desired.
...
\[
\vdash_{\rm{II}}  \HT{p[x:=t_0]}{S_1}{r[x:=t_0]}
\mbox{   and   }
\vdash_{\rm{II}}  \HT{r[x:=t_0]}{S_2}{q[x:=t_0]}.
\]
Then by the COMPOSITION  rule, 
$\vdash_{\rm{II}}  \HT{p[x:=t_0]}{S}{q[x:=t_0]}$, as desired.
...
By the induction hypothesis,
\[
 \vdash_{\rm{II}} \HT{(p \A B)[x:=t_0]}{S_1}{q[x:=t_0]}  \mbox{ and }
 \vdash_{\rm{II}} \HT{(p \A \neg B)[x:=t_0]}{S_2}{q[x:=t_0]}.
\]
Since $x \not\in var(B)$, the CONSEQUENCE rule yields
\[
 \vdash_{\rm{II}} \HT{p[x:=t_0] \A B}{S_1}{q[x:=t_0]}  \mbox{ and }
 \vdash_{\rm{II}} \HT{p[x:=t_0]) \A \neg B}{S_2}{q[x:=t_0]}.
\]
By the CONDITIONAL rule,
$\vdash_{\rm{II}} \HT{p[x:=t_0]}{S}{q[x:=t_0]}$,
as desired.
...
$p \ra p_{0}$, $p_{0} \ra t \ge 0$,  $(p_{0} \A \neg B) \ra q$
hold. Thus 
$p[x:=t_0] \ra p_{0}[x:=t_0]$, $p_{0}[x:=t_0] \ra t[x:=t_0] \ge 0$,  
$(p_{0} \A \neg B)[x:=t_0] \ra q[x:=t_0]$.
By the assumptions about $t, t_0, z$, we have
$var(t) \subseteq pvar$ and $x, z \in lvar$ with $z \not\in var(B)$.

KRA: (13-02-26) ??? $x$ CAN BE A PROGRAM VARIABLE! THIS IS IMPORTANT IN THE WAY THE SUBSTITUTION
RULE IS USED IN THE FINAL STEP OF THE PROOF OF (v) OF LEMMA 3 (PAGE 23)
...
By the induction hypothesis,
\[
 \vdash_{\rm{II}} \HT{(p_0 \A B \A t = z)[x:=t_0]}{S_0}{(p_0\A t < z)[x:=t_0]}.
\]
By the assumptions about $t, t_0, z$ and the CONSEQUENCE RULE,
\[
 \vdash_{\rm{II}} \HT{p_0[x:=t_0] \A B \A t = z}{S_0}{p_0[x:=t_0]\A t < z},
\]
and also $p_{0}[x:=t_0] \ra t \ge 0$ and $p_{0}[x:=t_0] \A \neg B \ra q[x:=t_0]$.
So the LOOP II rule yields 
\[
 \vdash_{\rm{II}} \HT{p_0[x:=t_0]}{S}{p_0[x:=t_0] \A \neg B}.
\]
By the CONSEQUENCE rule $\vdash_{\rm{II}} \HT{p[x:=t_0]}{S}{q[x:=t_0]}$, as
desired.
\HB
\end{proof}
...
KRA: (13-02-26) I WOULD JUST WRITE HERE 'II' INSTEAD OF 'proof system II', ETC.
...
We illustrate it by one illustrative case: proof system II $\approx$ proof system III.

Using Lemma~\ref{lem:derivability}(i) and Lemma~\ref{lem:admissibility}(ii), 
we instantiate Lemma~\ref{lem:equ} with $T$ being the proof system from
Appendix~\ref{sec:I} without the LOOP I rule:
\[
 T \cup \mathrm{LOOP\ II}\ \preceq\ T \cup \mathrm{LOOP\ III},
\]
i.e., proof system II $\preceq$ proof system III.
Using Lemma~\ref{lem:derivability}(ii) and Lemma~\ref{lem:admissibility}(iii), 
we instantiate Lemma~\ref{lem:equ} with $T$ as above:
\[
 T \cup \mathrm{LOOP\ III}\ \preceq\ T \cup \mathrm{LOOP\ II},
\]
i.e., proof system III $\preceq$ proof system II.
So altogether proof system II $\approx$ proof system III.
