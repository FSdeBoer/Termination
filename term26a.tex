...
\NI
KRA: (28-01): THIS SUBSECTION SHOULD BE CHANGED, SEE COMMENTS IN 4.1
...
Given a proof system $T$ and a proof rule (in short: a rule) $R$
abbreviate $T \cup \C{R}$ to $T \cup R$ and given two proof systems
$T_1$ and $T_2$ abbreviate the statement that they are equivalent to
$T_1 \approx T_2$.
...
\begin{lemma} \label{lem:equ}
  Consider a set of rules $\AUX \cup \C{R_1, R_2}$ and a proof system $T$.

  Suppose that for $k \in \C{1,2}$

  \begin{enumerate}[(i)]
  \item $R_k$ can be derived from $R_{3-k}$ using $AUX$, 

  \item each rule from $\AUX$ is admissible in $T \cup R_k$.    
  \end{enumerate}

  Then $T \cup R_1 \approx T \cup R_2$.
\end{lemma}

\begin{proof}
Fix $k \in \C{1,2}$.
  By $(i)$ with $k$ and $3-k$ interchanged and Lemma \ref{lem:AUX}$(ii)$

\[
T \cup \AUX \cup R_k \approx T \cup \AUX \cup R_1 \cup R_2
\]
and by $(ii)$ and Lemma \ref{lem:AUX}$(i)$
\[
    T \cup R_k \approx T \cup \AUX \cup R_k,
\]
so
\[
    T \cup R_k \approx T \cup \AUX \cup R_1 \cup R_2,
\]
from which letting $k$ vary over both 1 and 2 the claim follows.
\HB
\end{proof}
...
This rule generalized the following rule from \cite{ABO09}.
\III
...
KRA: (28-01). In term26.tex there is Theorem 1. The above auxiliary rules are admissible in each proof system I-V.

GIVEN THIS I (WRONGLY) ASSUMED THAT YOU PLANNED TO PROVE IT. THIS EXPLAINS THE VERSION WHICH IS BELOW UNTIL END.
I SHALL PREPARE, HOPEFULLY TOMORROW, AN ALTERNATIVE VERSION THAT TAKES CARE OF FRANK'S CONCERNS.
THIS WILL REQUIRE A REPLACEMENT OF LEMMAS 1 and 2.

In view of Lemma \ref{lem:equ} to prove Theorem \ref{thm:abo} it
suffices to establish the following two results, where $\AUX$ consists
of all listed auxiliary rules together with the CONSEQUENCE rule (that
also is an adaptation rule).

\begin{theorem} \label{thm:aux1}
  Each LOOP rule II-V can be derived from any other LOOP
rule II-V using $\AUX$.
\end{theorem}

\begin{theorem} \label{thm:aux2}
Each rule from $\AUX$ is admissible in each proof system II-V.
\end{theorem}

The proofs are given, respectively, in Appendices~\ref{sec:aux1} and \ref{sec:aux2}.
