...
NAMES OF TWO SUBSECTIONS AND NOTION OF DERIVIBILITY ADDED
...
\]
is called \bfe{derivable} \emph{PR}  if
\[
...
The application of a derivable rule can  be replaced without
...
The same holds for derivability:
The proof systems $\mathit{PR} \cup \{R_1\}$ and
$\mathit{PR} \cup \{R_2\}$ are equivalent iff $R_1$ is derivable  in
...
Let $p(x)$ denote the paramterized extension of
$\mbox{\it wp}(\WDD{B}{S},q)$.
...
In \cite{ABO90} completeness is discussed for proof system III
(for the same deterministic language).
Besides the expressibility of the standard weakest precondition
this completeness proof requires the expressibility of the
bound function as a term of the assertion language.
For this however the standard coding techniques do not suffice.
Instead it requires an extension of the assertion language
with recursive functions or general Skolem functions as used
in the derivability of the LOOP V rule in the proof system III.
